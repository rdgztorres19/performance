...
- **Concept:** *Branch misprediction* is when the CPU guesses the outcome of a conditional (if/else, loop) to keep the pipeline full, but the guess is wrong. The *pipeline* is the sequence of stages (fetch, decode, execute, etc.) that each instruction goes through; the CPU overlaps many instructions in flight at once.
- **Why:** The CPU must guess which path to take before the condition is resolved. If it guessed wrong, every instruction already fetched/decoded for the wrong path is useless and must be discarded. The pipeline then has to restart from the correct path. With 10–20 stages, that can mean 10–20 cycles (or more) lost per misprediction. In tight loops with unpredictable branches, this cost adds up and can account for a large fraction of execution time.
